...
import { Badge, Button, Modal, Select } from "@/design-system/code-companions-0f8a99";
...
const LEADER_NAV: NavGroup[] = [
  ...
  {
    label: "Records",
    items: [{ label: "Activity history", href: "/app/activity", icon: <History size={16} /> }],
  },
];

const DISTRICT_NAV: NavGroup[] = [
  {
    label: "District",
    items: [
...
const ACCOUNT_NAME: Record<DemoRole, string> = {
  teacher: "Ms. Rivera",
  ...
  return (
    <div className="app-frame">
      ...
      <div className="app-body">
        <nav className="shell-sidebar" aria-label="Workspace navigation">
          <Link to="/app" className="shell-sidebar__brand">
            <span className="wordmark__mark" aria-hidden="true" />
            <span>
              <span style={{ fontFamily: "var(--font-display)" }}>Hologram</span>
              <span className="shell-sidebar__brand-sub">{ROLE_LABEL[role]}</span>
            </span>
          </Link>

          {groups.map((group) => (
            <div key={group.label}>
              <div className="shell-group-label">{group.label}</div>
          ...
          <div className="shell-sidebar__footer">
            <Link to="/launch" className="shell-nav__item">
              <Share2 size={16} />
              <span>Simulated launch</span>
            </Link>
            <div className="shell-account">
              <span className="shell-account__avatar" aria-hidden="true">
                {initials(ACCOUNT_NAME[role])}
              </span>
              <span>
                <span className="shell-account__name">{ACCOUNT_NAME[role]}</span>
                <br />
                <span className="shell-account__role">{ROLE_LABEL[role]} · demo</span>
              </span>
            </div>
          </div>
        </nav>
        ...
        <div className="app-column">
          <header className="app-topbar">
            <strong style={{ fontSize: "var(--fs-body-sm)" }}>{activeLabel}</strong>
